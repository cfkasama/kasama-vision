...
export default async function RealizedPage(){
  ...
  return <div><h1 className="text-xl font-bold mb-3">実現一覧</h1>
    <ul className="list-disc pl-5">
      {posts.map(p=> <li key={p.id}><Link className="text-blue-600 hover:underline" href={`/posts/${p.id}`}>{p.title}</Link> <span className="text-xs text-gray-500">({p.realizedAt?.toLocaleString?.() || ""})</span></li>)}
    </ul>
  </div>;
}
